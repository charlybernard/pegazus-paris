...
\begin{table}[htbp]
    ...
    \end{tabular}
    \caption{Extract from a BAN file for the city of Paris\protect\footnotemark.}
    \label{tab:ban_sample}
\end{table}

\footnotetext{The address locations (provided by the \texttt{lat} and \texttt{lon} fields) in the BAN do not follow a single positioning rule. For each address, the method used to determine the location is specified (postal delivery point, entrance door, parcel centroid, etc.).}
