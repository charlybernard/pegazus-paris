...
SELECT DISTINCT ?address ?addressLabel ?geom WHERE {
    ...
    ?lm a peg:Landmark ; peg:isLandmarkType ltype:Thoroughfare; rdfs:label ?streetName.
    FILTER(LCASE(STR(?streetName)) = LCASE(?streetLabel) && LANG(?streetName) = ?streetLang)
    ?address a peg:Address; peg:hasStep [a peg:AddressSegment; peg:relatum ?lm] ; rdfs:label ?addressLabel.
    OPTIONAL {?address peg:targets [peg:hasAttribute [peg:isAttributeType atype:Geometry ; peg:hasAttributeVersion [peg:versionValue ?geom]]].}
}
```
...
SELECT ?item ?addressLabel ?geom WHERE {
    ...
    peg:targets [peg:hasAttribute [peg:isAttributeType atype:Geometry ; peg:hasAttributeVersion [peg:versionValue ?geom]]].
...
SELECT DISTINCT ?addr ?addrLabel ?geom WHERE {
    ...
    ?addr a peg:Address ; rdfs:label ?addrLabel ; peg:targets [peg:hasAttribute [peg:isAttributeType atype:Geometry ; peg:hasAttributeVersion [peg:versionValue ?geom]]] .
