...
def create_landmark_relation(
    ...
    g.add((landmark_relation_uri, RDF.type, np.PEG[lr_class]))
    g.add((landmark_relation_uri, np.PEG["isLandmarkRelationType"], landmark_relation_type))
    g.add((landmark_relation_uri, np.PEG["locatum"], locatum_uri))
...
def create_attribute_change(
    ...
    for version in made_effective_versions_uris:
        g.add((change_uri, np.PEG["makesEffective"], version))
...
def create_attribute_version_with_changes(g:Graph, attribute_uri:URIRef, value:Literal, resource_namespace:Namespace,
    ...
    g.add((outdates_change_uri, np.PEG["outdates"], attr_vers_uri))
    g.add((makes_effective_change_uri, np.PEG["makesEffective"], attr_vers_uri))
...
def create_address(g:Graph, address_uri:URIRef, address_label:Literal, address_segments_list:list[URIRef], target_uri:URIRef)->None:
    ...
    :param g: RDF graph
    :param address_uri: URI of the address to create
    :param address_label: label of the address to create
    :param address_segments_list: list of URIs of the address segments (landmark relations
    :param target_uri: URI of the target of the address (a landmark or a spatial unit)
    :return: None
    """

    g.add((address_uri, RDF.type, np.PEG["Address"]))
    g.add((address_uri, RDFS.label, address_label))
    g.add((address_uri, np.PEG["targets"], target_uri))
    g.add((address_uri, np.PEG["firstStep"], address_segments_list[0]))

    prev_addr_seg = address_segments_list[0]
    for addr_seg in address_segments_list[1:]:
        g.add((prev_addr_seg, np.PEG["nextStep"], addr_seg))
