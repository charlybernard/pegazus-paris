...
def parse_sparql_results(results: dict, variables: list[str]) -> list[dict]:
    """
    Parses SPARQL JSON results from GraphDB into a list of dictionaries 
    containing RDFLib elements.
    
    :param results: The JSON dictionary returned by the query execution.
    :param variables: A list of string names of the variables to extract (without '?').
    :return: A list of dictionaries where keys are variable names and values are RDFLib objects.
    """
    ...
    for binding in bindings:
        ...
        for var in variables:
            raw_element = binding.get(var)
            if raw_element:
                # Converts the JSON element (uri, literal, bnode) to an RDFLib object
                row[var] = gr.convert_result_elem_to_rdflib_elem(raw_element)
            else:
                row[var] = None
...
def fetch_sparql_data(query, graphdb_url, repository_name, variables):
    """
    Fetches data from a GraphDB repository using a SPARQL SELECT query and parses the results into RDFLib elements.
    ...
    :param variables: A list of variable names (without '?') to extract from the query results.
    :return: A list of dictionaries where each dictionary corresponds to a row of results, with keys as variable names and values as RDFLib elements (URIRef, Literal, BNode).
"""
    try:
        # 1. Executes the SPARQL SELECT query and gets results in JSON format
        results_json = run_select_query_to_json(query, graphdb_url, repository_name)
        
        # 2. Parses the JSON results to convert them into RDFLib elements
        parsed_data = parse_sparql_results(results_json, variables)
...
def format_sparql_values(configs, keys_to_include):
    """
    Formats a list of dictionaries (configs) into a string suitable for SPARQL VALUES clause,
    including only the specified keys.
    Parameters:
    - configs (list of dict): A list of dictionaries where each dictionary represents a row of variable bindings.
    - keys_to_include (list of str): A list of keys to include in the output. Each key corresponds to a variable in the SPARQL query.
    Returns:
    - str: A string formatted for the SPARQL VALUES clause, containing only the specified keys from each dictionary in the configs list.
    """
    rows = []
    for cfg in configs:
        # On récupère la représentation n3() pour chaque clé demandée
        row_str = " ".join([cfg[k].n3() for k in keys_to_include])
...
def add_named_graph_prefix_to_repository(graphdb_url:URIRef, repository_name:str, prefix:str):
    """
    Add a new namespace prefix for named graph URIs to a repository.

    Parameters:
    - graphdb_url (URIRef): The base URL of the GraphDB instance.
    - repository_name (str): The name of the repository to add the prefix to.
    - prefix (str): The prefix to associate with the named graph's URI.

    Returns:
    - Response object: The response object returned by the requests.put call, which contains the status code and content of the request.

    Notes:
    - This function constructs the URI of the named graph and sends a PUT request to add it as a namespace with the specified prefix to the repository.
    """
    
    named_graph_uri = get_named_graph_uri_from_name(graphdb_url, repository_name, "")
    return add_prefix_to_repository(graphdb_url, repository_name, named_graph_uri, prefix)
